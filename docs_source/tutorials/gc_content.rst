...
A third version shows a more scalable version of the same code, which uses the "read_chunks" function with the fasta file to get a generator that reads in a single chromosome sequence at a time, allowing it to handle full-genome data with limited memory footprint:
