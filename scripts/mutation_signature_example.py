...
def main(vcf_filename: str, fasta_filename: str, out_filename: str = None, flank: int = 1, has_numeric_chromosomes=True, genotyped=False):
    ...
    counts = None
    for chunk in bnp.open(vcf_filename, buffer_type=buffer_type).read_chunks():
        variants = genome.get_locations(chunk, has_numeric_chromosomes=has_numeric_chromosomes)
        if counts is None:
            counts = count_mutation_types_genomic(variants, sequence, genotyped=genotyped)
        else:
            counts += count_mutation_types_genomic(variants, sequence, genotyped=genotyped)
...
def pipeline(vcf_filename: str, fasta_filename: str, signature_filename: str, out_filename: str, has_numeric_chromosomes: bool=True, genotyped: bool=False):
    counts = main(vcf_filename, fasta_filename, out_filename=out_filename, has_numeric_chromosomes=has_numeric_chromosomes, genotyped=genotyped)
    signature_matrix = read_matrix(signature_filename)
